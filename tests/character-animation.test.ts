...
assert(!controller.playOneShot("shoot", { restartIfActive: false }), "Automatic fire does not restart active Shoot");
near(controller.getSnapshot().actions.find(action => action.state === "shoot")!.time, shootAction.time, "Suppressed repeat keeps Shoot time");
controller.playOneShot("shoot");
...
const beforeFadeOutShot = bone.position.x;
assert(!controller.playOneShot("shoot", { restartIfActive: false }), "Automatic fire waits until outgoing Shoot has faded out");
near(bone.position.x, beforeFadeOutShot, "Suppressed fade-out restart keeps the pose");
controller.update(0.12);
near(controller.getSnapshot().actions.find(action => action.state === "idle")!.weight, 1, "Standing Shoot returns fully to Idle");
assert(controller.playOneShot("shoot"), "Shoot starts before reload interruption");
controller.update(0.1);
assert(controller.stopOneShot("shoot"), "Reload can end an active Shoot");
assert(controller.getSnapshot().state === "idle", "Reload returns to current locomotion");
controller.update(0.12);
near(controller.getSnapshot().actions.find(action => action.state === "idle")!.weight, 1, "Reload interruption completes its blend");
