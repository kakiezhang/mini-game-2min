...
- PC 键盘/指针与移动端摇杆操作。
- 手动冲锋枪、射线命中、主角 Shoot 动作、弹匣换弹、后备弹药和地图补给。按下射击立即启动单发 Shoot，约 0.10 秒后在动作开火点扣弹、生成枪口／子弹表现并结算命中；按住仍每 0.20 秒一发，每发以短混合重新触发一次后坐力。原始 Shoot 包含多次后坐力，游戏只取前 0.30 秒；开始换弹时退出射击姿势并取消尚未射出的子弹。
