...
在「Idle ↔ Walk 切换测试」中，按住空格或「按住移动」按钮播放 Walk，松开回 Idle；「自动往返」每 2 秒动画时间切换一次。速度滑块同时影响动作和 0.12 秒过渡，调到 0.25× 可观察混合权重及起步／停步姿势。支持暂停、触控移出按钮后释放、失焦释放；切回单动作模式仍可拖动时间轴。缺少 Idle 或 Walk 的模型禁用切换测试。验收时检查身体不歪斜、手臂不抽动、脚底不突然升降，以及快速反复按松后正常回 Idle。此模式复用游戏的 `CharacterAnimationController`，不会模拟角色在地图上的位移；最后仍需在游戏中用键盘和触控验收。

`npm run test:animation` 检查初始姿势、权重归一化、中途反向切换、循环、单 Walk 模型兼容及实例独立性。`npm run build` 同时构建游戏和 `character-preview.html`。
