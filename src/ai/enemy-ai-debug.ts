...
export class EnemyAiDebugLayer {
  ...
  update(runtime: EnemyAiRuntime, pose: EnemyAiDebugPose) {
    ...
    const label = [
      ...
      `zone ${runtime.homeZone} · slot ${runtime.approachSlotId ?? "-"}/${runtime.approachSlotFailure} · path ${runtime.pathIndex}/${runtime.path.length} · seen ${Number.isFinite(runtime.lastSeenAt) ? runtime.lastSeenAt.toFixed(1) : "-"} · stuck ${stuckDuration} ${runtime.failure}/${runtime.failureCause}`,
    ].join("\n");
